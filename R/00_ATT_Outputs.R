...
source('./R/15_carc_ages.R') # Table 7 - carcass ages by return year
# Table 8 - R/S, SAR, PP
source('./R/T9andT11_steelhead.R')# Tables 9 & 11 - sth trapping and escapement
...
T3_T6 <- full_join(T3_trib_esc, T6_tmp)
write_xlsx(T3_T6, path = './data/outputs/templates/T3-T6-GRSME.xlsx')

write_xlsx(T4_disp, path = './data/outputs/templates/T4-GRSME.xlsx')

T5_T7 <- full_join(T5_sgs_stuff, pAge_carc)
write_xlsx(T5_T7, path = './data/outputs/templates/T5-T7-GRSME.xlsx')

T9_T11 <- full_join(T9_sth_disp, T11_sth_weirstuff) %>%
  .[, c(2, 1, 3:ncol(.))]
write_xlsx(T9_T11, path = './data/outputs/templates/T9-T11-GRSME.xlsx')
